...
public interface OrderDetailRepository extends JpaRepository<OrderDetail, Long> {
     ...
     * @return
     */
    @Query(value = "select max(order_id) order_id, max(order_status)order_status, max(customer_id)customer_id, max(created) created, max(updated) updated, sum(total_purchase) total_purchase from order_detail where customer_id= ?1 and month(created)>= ?2 group by year(CREATED),month(created) order by year(created),month(created)",nativeQuery = true)
